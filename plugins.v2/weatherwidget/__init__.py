...
# 截图设备参数保持为插件本地常量，避免依赖 Playwright 自带设备注册表和旧浏览器内核。
...
class WeatherWidget(_PluginBase):
    ...
    def __take_screenshots(self):
        ...
        try:
            ...
            if self.__take_screenshots_by_cloakbrowser(screenshot_devices=screenshot_devices,
                                                       color_scheme=color_scheme,
                                                       start_time=start_time):
                return
            self.__take_screenshots_by_playwright(screenshot_devices=screenshot_devices,
                                                  color_scheme=color_scheme,
                                                  start_time=start_time)
        except Exception as e:
            logger.error(f"take_screenshots failed: {str(e)}")

    def __take_screenshots_by_cloakbrowser(self,
                                           screenshot_devices: dict,
                                           color_scheme: str,
                                           start_time: datetime) -> bool:
        """优先使用主程序新浏览器内核截图，不可用时交给 Playwright 兼容路径。"""
        try:
            __import__("cloakbrowser")
        except Exception as e:
            logger.warning(f"CloakBrowser 不可用，回退 Playwright 截图: {e}")
            return False

        logger.info("正在准备截图服务，CloakBrowser 浏览器内核启动中")
        for key, device in screenshot_devices.items():
            try:
                logger.info(f'{key} 正在启动 CloakBrowser screenshot ...')
                if not self.__screenshot_element_by_cloakbrowser(key=key, device=device, color_scheme=color_scheme):
                    logger.warning(f"{key} CloakBrowser screenshot failed，回退 Playwright 截图")
                    return False
                elapsed_time = datetime.now() - start_time
                logger.info(f'运行完毕，用时 {elapsed_time.total_seconds()} 秒')
            except Exception as e:
                logger.warning(f"CloakBrowser screenshot failed，回退 Playwright 截图: {str(e)}")
                return False
        return True

    def __take_screenshots_by_playwright(self, screenshot_devices: dict, color_scheme: str, start_time: datetime):
        """使用旧版 Playwright 浏览器内核截图，兼容尚未切换到 CloakBrowser 的主程序环境。"""
        if not sync_playwright:
            logger.error(f"Playwright 兼容内核不可用，无法继续截图: {PLAYWRIGHT_IMPORT_ERROR}")
            self.__update_with_log_screenshot_time(current_time=None)
            return

        with sync_playwright() as playwright:
            logger.info("正在准备截图服务，Playwright 兼容内核启动中")
            with playwright.chromium.launch(headless=True, proxy=settings.PROXY_SERVER) as browser:
                for key, device in screenshot_devices.items():
                    try:
                        logger.info(f'{key} 正在启动 Playwright screenshot ...')
                        self.__screenshot_element_by_playwright(browser=browser,
                                                                key=key,
                                                                device=device,
                                                                color_scheme=color_scheme)
                        elapsed_time = datetime.now() - start_time
    ...
    @staticmethod
    def __get_cloakbrowser_context_options(device: dict, color_scheme: str) -> dict:
        """生成 CloakBrowser 上下文参数，兼容主程序 v2.13+ 预下载的浏览器内核。"""
    ...
    @staticmethod
    def __launch_cloakbrowser_context(device: dict, color_scheme: str):
        """启动用于天气截图的 CloakBrowser 上下文。"""
        from cloakbrowser import launch_context

        return launch_context(headless=True,
                              ...
                              **WeatherWidget.__get_cloakbrowser_context_options(device=device,
                                                                                 color_scheme=color_scheme))

    def __screenshot_element_by_cloakbrowser(self, key: str, device: dict, color_scheme: str = 'light') -> bool:
        """使用 CloakBrowser 执行单个截图任务。"""
        current_time = datetime.now(tz=pytz.timezone(settings.TZ))
        ...
        page = None
        try:
            context = self.__launch_cloakbrowser_context(device=device, color_scheme=color_scheme)
    ...
    def __screenshot_element_by_playwright(self, browser, key: str, device: dict, color_scheme: str = 'light'):
        """使用 Playwright 执行单个截图任务，保留低版本主程序的兼容能力。"""
        current_time = datetime.now(tz=pytz.timezone(settings.TZ))
        timestamp = current_time.strftime("%Y%m%d%H%M%S")
        selector = ".c-city-weather-current"
        image_path = IMAGES_PATH / f"{self.__get_screenshot_image_pre_path(key=key)}_{timestamp}.png"
        context_options = self.__get_cloakbrowser_context_options(device=device, color_scheme=color_scheme)

        logger.info(f"开始加载 {key} 页面: {self._weather_url}")
        self.__update_with_log_screenshot_time(current_time=current_time)
        with browser.new_context(**context_options) as context:
